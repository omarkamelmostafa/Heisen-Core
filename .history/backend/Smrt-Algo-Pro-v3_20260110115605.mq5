...
void DrawDashboard(double currentPrice, int index);
...
void DrawDashboard(double currentPrice, int index)
{
   string nameBg = "Dash_Bg";
   string nameText = "Dash_Text";
   
   if(!InpEnableDashboard) return;
   
   // Simple Panel
   // Logic for Position (derived from last signal)
   string posText = "No Signal";
   color posColor = clrGray;
   
   // Retreive last signal from state
   int type = State_SignalType[index];
   if(type == 1 || type == 2) { posText = "Buy"; posColor = clrLimeGreen; }
   if(type == 3 || type == 4) { posText = "Sell"; posColor = clrRed; }
   
   // Create/Update Objects
   if(ObjectFind(0, nameBg) < 0) {
      ObjectCreate(0, nameBg, OBJ_RECT_LABEL, 0, 0, 0);
      ObjectSetInteger(0, nameBg, OBJPROP_CORNER, CORNER_RIGHT_LOWER);
      ObjectSetInteger(0, nameBg, OBJPROP_XDISTANCE, 10);
      ObjectSetInteger(0, nameBg, OBJPROP_YDISTANCE, 50);
      ObjectSetInteger(0, nameBg, OBJPROP_XSIZE, 150);
      ObjectSetInteger(0, nameBg, OBJPROP_YSIZE, 100);
      ObjectSetInteger(0, nameBg, OBJPROP_BGCOLOR, clrBlack);
      ObjectSetInteger(0, nameBg, OBJPROP_BORDER_TYPE, BORDER_FLAT);
      ObjectSetInteger(0, nameBg, OBJPROP_COLOR, clrWhite);
   }
   
   if(ObjectFind(0, nameText) < 0) {
      ObjectCreate(0, nameText, OBJ_LABEL, 0, 0, 0);
      ObjectSetInteger(0, nameText, OBJPROP_CORNER, CORNER_RIGHT_LOWER);
      ObjectSetInteger(0, nameText, OBJPROP_XDISTANCE, 20);
      ObjectSetInteger(0, nameText, OBJPROP_YDISTANCE, 120);
      ObjectSetInteger(0, nameText, OBJPROP_COLOR, clrWhite);
      ObjectSetInteger(0, nameText, OBJPROP_FONTSIZE, 10);
   }
   
   string text = "SMRT Algo Pro\n" + 
                 "Position: " + posText + "\n" +
                 "Price: " + DoubleToString(currentPrice, _Digits);
                 
   ObjectSetString(0, nameText, OBJPROP_TEXT, text);
   
   // Create colored indicator for position
   string namePos = "Dash_PosColor";
   if(ObjectFind(0, namePos) < 0) {
       ObjectCreate(0, namePos, OBJ_RECT_LABEL, 0, 0, 0);
       ObjectSetInteger(0, namePos, OBJPROP_CORNER, CORNER_RIGHT_LOWER);
       ObjectSetInteger(0, namePos, OBJPROP_XDISTANCE, 80);
       ObjectSetInteger(0, namePos, OBJPROP_YDISTANCE, 90);
       ObjectSetInteger(0, namePos, OBJPROP_XSIZE, 50);
       ObjectSetInteger(0, namePos, OBJPROP_YSIZE, 20);
   }
   ObjectSetInteger(0, namePos, OBJPROP_BGCOLOR, posColor);
}
