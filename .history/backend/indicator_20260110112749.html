...
reversalBandsEnabled = input.bool(false, title="Reversal Bands",
group="INDICATORS")
...
// Plot buy/sell signals(MUDAR CORES)
...
// ========== TP/SL BREAKOUT LABELS CORRIGIDOS ========== //
if longTp1Breakout
    label.new(bar index, high, text="●", color=color.rgb(0, 255, 0, 100),
textcolor=longtp, style=label.style_label_down, size.small)
if longTp2Breakout
    label.new(bar index, high, text="●", color=color.rgb(0, 255, 0, 100),
textcolor=longtp, style=label.style_label_down, size.small)
if longTp3Breakout
...
if shortTp1Breakout
    label.new(bar index, low, text="●", color=color.rgb(0, 255, 0, 100),
textcolor=shorttp, style=label.style_label_up, size.small)
if shortTp2Breakout
    label.new(bar index, low, text="●", color=color.rgb(0, 255, 0, 100),
textcolor=shorttp, style=label.style_label_up, size.small)
if shortTp3Breakout
...
labelTpSl(cond, y, txt, bgcolor, txtColor) =>
    ...
    if riskmanage_bool and cond
        labelTpSl := label.new(bar index + 1, y, txt, xloc=xloc.bar_index,
...
signal_type == 4 ? ta.barssince(valid_Sbear) : 0
    ...
    if riskmanage_bool and cond and count >= 0
        lineTpSl := line.new(bar_index - count, y, bar index + 1,
xloc=xloc.bar_index, extend=extend.none, color=color, style=style)
        line.delete(lineTpSl[1])

// ========== LINHAS E LABELS RISCOS FIXADOS NO GRÁFICO ========== //
...
securityNoRep(sym, timeframeStr, src) =>
    bool bull3 = na
...
src, barmerge.gaps_off, barmerge.lookhead_on) : bull3
...
lineWidth1 = input.int(0, title="Line Width", minval=0, maxval=4, group="Trend
Lines")
expandTrend = input(false, "Extend Trend Line", group="Trend Lines")
...
lineStyle1 = (styleOption == "Dotted -") ? line.style_dotted :
           ...
           (styleOption == "Arrow Both ↔︎
                                        ") ? line.style_arrow_both :
             line.style_solid

length1 = input.int(80, minval=1, title="Length 1", display=display.none,
group='reversal Bands')
length2 = input.int(80, minval=1, title="Length 2", display=display.none,
group='reversal Bands')
length3 = input.int(80, minval=1, title="Length 3", display=display.none,
group='reversal Bands')
...
exp = input.bool(false, title="Use Exponential MA", display=display.none,
group='reversal Bands')
...
atrlength1 = input.int(2000, title="ATR Length 1", display=display.none,
group='reversal Bands')
atrlength2 = input.int(2000, title="ATR Length 2", display=display.none,
group='reversal Bands')
atrlength3 = input.int(2000, title="ATR Length 3", display=display.none,
group='reversal Bands')
...
u1 = plot(plotKeltnerChannels ? upper1 : na, color=color.new(#787b86, 70),
title="Upper Band 1")
l1 = plot(plotKeltnerChannels ? lower1 : na, color=color.new(#787b86, 70),
title="Lower Band 1")

u2 = plot(plotKeltnerChannels ? upper2 : na, color=color.new(#787b86, 70),
title="Upper Band 2")
l2 = plot(plotKeltnerChannels ? lower2 : na, color=color.new(#787b86, 70),
title="Lower Band 2")

u3 = plot(plotKeltnerChannels ? upper3 : na, color=color.new(#787b86, 70),
title="Upper Band 3")
l3 = plot(plotKeltnerChannels ? lower3 : na, color=color.new(#787b86, 70),
title="Lower Band 3")
...
offset_input = input.float(30, title="Risk Management", minval=0.1, step=0.1,
group='TP/SL')
...
percWidth(period, width) =>
    range = ta.highest(high, period) - ta.lowest(low, period)
    range * width / 100
...
lowestpl - zonePerc, xloc.bar_index, expandSR ? extend.both : extend.right, na)
    ...
    for x = 0 to array.size(sr_lines) - 1
        ...
        if not na(array.get(sr_levs, x)) and enableSR
            ...
            if useZones
                array.set(sr_linesH, x, line.new(bar_index - 355,
array.get(sr_levs, x) + zonePerc, bar_index, array.get(sr_levs, x) + zonePerc,
xloc.bar_index, expandSR ? extend.both : extend.right, na))
                array.set(sr_linesL, x, line.new(bar_index - 355,
array.get(sr_levs, x) - zonePerc, bar_index, array.get(sr_levs, x) - zonePerc,
xloc.bar_index, expandSR ? extend.both : extend.right, na))
...
G_POI = 'Supply/Demand'
POIATR = ta.atr(300)
ColorSupply = input.color(#9d0e4a, title='Supply', group=G_POI)
...
group=G_POI)
SwingLength = input.int(10, title='High/Low Swing Length', group=G_POI, minval=1,
maxval=50)
HistoryDemandKepp = input.int(21, title='History to maintain', minval=5, maxval=50,
group=G_POI)
BoxWidth = input.float(10, title='Supply/Demand Box Width', group=G_POI, minval=1,
maxval=10, step=0.5)
...
box_right, bottom = box_bottom, border_color = ColorSupplyOutline, bgcolor =
ColorSupply, extend=MaxExtention?extend.right:extend.none, text='', text_color =
ColorLabelPOI, text_size = size.small, xloc = xloc.bar_index))

    else if box_type == -1 and okay_to_draw and POIswitch
        box.delete(array.get(box_array, array.size(box_array) - 1) )
        ArrayAddPopF(box_array, box.new( left = box_left, top = box_top, right =
box_right, bottom = box_bottom, border_color = ColorDemandOutline, bgcolor =
ColorDemand, extend=MaxExtention?extend.right:extend.none, text='', text_color=
ColorLabelPOI, text_size= size.small, xloc = xloc.bar_index))
...
box.get_bottom(array.get(box_array,i))) / 2
                ...
                box.set_text_color(array.get(bos_array,0),
...
var SwingHighValues = array.new_float(5,0.00)
...
if not na(SwingHigh)
    ...
    ArrayAddPopF(SwingHighValues, SwingHigh)
    ArrayAddPopF(SwingHighBNS, bar_index[SwingLength])
    SupplyDemandF(SwingHighValues, SwingHighBNS, SupplyBoxCurrent,
...
75), style=plot.style_circles)
p3 = plot(sarzone_plot, "Zone Line", color=color.new(close > sarone ? #00ff0a :
#ff0014, 75), style=plot.style_circles)

// Constants
color_CLEAR = color.rgb(0,0,0,100)
...
if marketStructure
    // Swing Points
    if hh
        label.new(bar_index - swingSize, pivHi, 'HH', color=color_CLEAR,
style=label.style_label_down, textcolor=chart.fg_color, size=size.small)
    if lh
        label.new(bar_index - swingSize, pivHi, 'LH', color=color_CLEAR,
style=label.style_label_down, textcolor=chart.fg_color, size=size.small)
    if hl
        label.new(bar_index - swingSize, pivLo, 'HL', color=color_CLEAR,
style=label.style_label_up, textcolor=chart.fg_color, size=size.small)
    if ll
        label.new(bar_index - swingSize, pivLo, 'LL', color=color_CLEAR,
style=label.style_label_up, textcolor=chart.fg_color, size=size.small)
...
prevHigh, prevBreakoutDir == -1 ? 'CHOCH' : 'BOS', color=color_CLEAR,
...
prevBreakoutDir == 1 ? 'CHOCH' : 'BOS', color=color_CLEAR, textcolor=bosColor,
...
color=color.new(#00e5ff, 70), size=size.tiny, title="Up Reversal")
plotshape(reversal and isAbove70, style=shape.diamond, location=location.abovebar,
color=color.new(#ff0066, 70), size=size.tiny, title="Down Reversal")
...
plot(zlemaPlot, title="ZLEMA", linewidth=2, color=color.new(zlemaColor, 0))

// Trend Lines with Dynamic Recalculation
