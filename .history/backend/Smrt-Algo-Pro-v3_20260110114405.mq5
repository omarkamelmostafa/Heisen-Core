...
int MainCalculation(int rates_total, int prev_calculated, const double &open[], const double &high[], const double &low[], const double &close[], const double &atr[])
{
   ...
   if(start < 1) {
       start = 1;
       // Initialize first elements to avoid garbage
   ...
   for(int i = start; i < rates_total; i++)
   {
      ...
      // SuperTrend Crossover Logic
      // Bull Crossover: Trend changes from Down(1) to Up(-1)
      bool crossOverST = (ST_Direction[i] == -1 && ST_Direction[i-1] == 1);
      // Bear Crossunder: Trend changes from Up(-1) to Down(1)
      bool crossUnderST = (ST_Direction[i] == 1 && ST_Direction[i-1] == -1);
      ...
      // Signal Definitions
      // Bull: Crossover AND close >= sma3 AND NOT(closeAboveEMA) AND maFilter
      bool bull = crossOverST && close[i] >= sma3 && !closeAboveEMA && maFilterCond;
      
      // Bear: Crossunder AND close <= sma3 AND (closeAboveEMA) AND maFilterBear
      // Note: Pine logic "not(not(...))" -> true
      bool bear = crossUnderST && close[i] <= sma3 && closeAboveEMA && maFilterCondBear;
      
      // Strong Bull: Crossover AND close >= sma3 AND (closeAboveEMA) AND maFilter
      bool Sbull = crossOverST && close[i] >= sma3 && closeAboveEMA && maFilterCond;
      
      // Strong Bear: Crossunder AND close <= sma3 AND NOT(closeAboveEMA) AND maFilterBear
