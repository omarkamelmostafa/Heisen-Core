...
describe("Suite G — Reset Password Tests", () => {
  ...
  it("G6: Password reset revokes all sessions", async () => {
    await registerAndVerify(app, emailService, TEST_USER);

    // Setup: Login on 2 separate agents (simulating two devices)
    ...
    // Reset password
    const resetToken = await getResetToken(TEST_USER.email);
