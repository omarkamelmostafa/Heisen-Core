...
describe("Suite C — Refresh Token Tests", () => {
  ...
  it("C6: Token reuse detection (rotates once, then reuse old)", async () => {
    const { agent, loginRes } = await registerVerifyAndLogin(app, emailService);

    // 1. Capture the initial refresh token from login
    const cookie1 = loginRes.headers["set-cookie"][0].split(';')[0]; 
    
    // 2. Perform first refresh (rotates cookie1 -> cookie2)
    const refreshRes1 = await agent.post("/api/v1/auth/refresh");
    expect(refreshRes1.status).toBe(200);
    const cookie2 = refreshRes1.headers["set-cookie"][0].split(';')[0];
    expect(cookie2).not.toBe(cookie1);

    // 3. Attempt to REUSE cookie1 (which is now revoked)
    // This should trigger the "nuclear" option in token-service.js
    ...
    // 4. Verify that cookie2 (the latest valid one) is NOW ALSO REVOKED
    const finalRes = await request(app)
      .post("/api/v1/auth/refresh")
      .set("Cookie", cookie2);
    
    expect(finalRes.status).toBe(401);
    expect(finalRes.body.message).toContain("revoked");
  });
});
