...
- [ ] **E9**: Production i18n Implementation (EN/AR).
Objective:
Make the entire application fully multilingual (English + Arabic) with proper RTL support and scalable translation architecture.

Implementation Steps:
1. Install and configure `next-intl`
2. Add locale routing using App Router
3. Create translation dictionaries (`en.json`, `ar.json`)
4. Wrap root layout with NextIntlProvider
5. Implement dynamic locale detection (cookie + URL)
6. Add RTL support when locale = "ar"
7. Refactor all UI text into translation keys
8. Add language switcher component
9. Ensure middleware handles locale redirection
10. Validate SSR compatibility and hydration stability
...
- RTL-safe layout styles (Tailwind or CSS logical properties)

Definition of Done:
App is fully usable in both English and Arabic with correct RTL layout and zero hardcoded strings.

- [x] **HOTFIX-01**: Fix build error "Module not found: Can't resolve './globals.css'
  - *Completed On: 2026-02-20*

./src/app/[locale]/layout.jsx (6:1)
"

Context:
After reverting previous tasks, the Next.js build is failing with:
"Module not found: Can't resolve './globals.css'"

This likely occurred due to:
- Reverted layout file changes
- Missing or moved globals.css file
- Incorrect import path in root layout or _app file

Error Scope:
The error happens during compilation and prevents the app from building.

Required Actions:
1. Locate where `globals.css` is imported (likely in `app/layout.tsx` or `pages/_app.tsx`)
2. Verify the actual file location:
   - `/app/globals.css`
   - `/styles/globals.css`
3. Restore the file if it was deleted during revert
4. Fix the import path to match the correct directory
5. Ensure Tailwind (if used) still imports correctly
6. Confirm the project builds successfully with no module resolution errors

Technical Constraints:
- Do NOT restructure unrelated files
- Only fix the missing CSS import and related build issues
- Preserve existing project architecture

Acceptance Criteria:
- `npm run build` completes without errors
- `globals.css` is correctly resolved and loaded
- No broken layout styles after the fix
