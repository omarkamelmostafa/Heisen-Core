...
export function AuthSubmitButton({
  ...
  const calculateDisabledState = () => {
    ...
    if (validateCode && formData[codeField]) {
      const code = formData[codeField];
      const isCodeIncomplete = code.some(
        (digit) => !digit || digit.toString().trim() === ""
      );
      if (isCodeIncomplete) return true;
    }

    // Validate password match and length
    if (validatePasswordMatch) {
      const password = formData[passwordField];
      const confirmPassword = formData[confirmPasswordField];
...
export const AuthSubmitButtons = {
  ...
  ResetPassword: ({ isLoading, ...props }) => (
    <AuthSubmitButton
      ...
      validateEmptyFields={["password", "confirmPassword"]}
      validatePasswordMatch={true}
      showDefaultIcon={true}
