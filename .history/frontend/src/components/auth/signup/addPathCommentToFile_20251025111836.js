...
//   const projectRoot = process.cwd(); // Gets the project root where you run the script

//   const files = fs.readdirSync(scriptDir);

//   files.forEach((file) => {
//     if (path.extname(file) === ".js" && file !== path.basename(__filename)) {
//       const filePath = path.join(scriptDir, file);
//       const content = fs.readFileSync(filePath, "utf8");

//       // Get relative path from project root and format it
//       const relativePath = path
//         .relative(projectRoot, filePath)
//         .replace(/\\/g, "/");
//       const pathComment = `// ${relativePath}\n`;

//       // Remove existing path comment if any, then add new one
//       const newContent = content.replace(/^\/\/ [^\n]+\n/, "") + content;
//       const finalContent = content.startsWith(pathComment)
//         ? content
//         : pathComment + content.replace(/^\/\/ [^\n]+\n/, "");
...
//   console.log("🎉 All JS files now have relative path comments!");
// }
