...
function addPathComments() {
  ...
  files.forEach((file) => {
    if (path.extname(file) === ".js" && file !== path.basename(__filename)) {
      ...
      const content = fs.readFileSync(filePath, "utf8");
      const pathComment = `// ${filePath.replace(/\\/g, "/")}\n`;

      if (!content.startsWith(pathComment)) {
        fs.writeFileSync(filePath, pathComment + content, "utf8");
        console.log(`✅ Added to: ${file}`);
      }
    }
  });

  console.log("🎉 All JS files in current folder have path comments!");
}
