// components/auth/production-error-trigger.jsx
"use client";

import { useState, useEffect } from "react";
import { Button } from "@/components/ui/button";
import { AlertTriangle, RefreshCw, Bug, Server } from "lucide-react";
import { isDevelopment } from "@/lib/environment";

// Real error-throwing components (for render-time errors)
function RuntimeErrorComponent() {
  throw new Error("Production runtime error - component render failure");
  return null;
}

function TypeErrorComponent() {
  const nullObject = null;
  // This will throw during render
  return <div>{nullObject.nonExistentProperty}</div>;
}

function AsyncErrorComponent() {
  const [data, setData] = useState(null);

  // Simulate async operation that fails during render
  if (!data) {
    // This simulates a real-world data fetching error during render
    throw new Error("Data fetching failed in production");
  }

  return <div>{data}</div>;
}

export function ProductionErrorTrigger() {
  const [errorMode, setErrorMode] = useState(null);
  const [simulateCrash, setSimulateCrash] = useState(false);

  // Don't render in development
  if (isDevelopment) {
    return null;
  }

  // Reset error state after a delay
  useEffect(() => {
    if (errorMode) {
      const timer = setTimeout(() => {
        setErrorMode(null);
      }, 5000);
      return () => clearTimeout(timer);
    }
  }, [errorMode]);

  // Real render-time error triggers
  const triggerRenderError = (type) => {
    setErrorMode(type);
  };

  // Simulate complete app crash (uncaught error)
  const triggerUncaughtError = () => {
    setTimeout(() => {
      throw new Error("Uncaught production error - app crash simulation");
    }, 100);
  };

  // Render the actual error components based on mode
  const renderErrorComponent = () => {
    switch (errorMode) {
      case "runtime":
        return <RuntimeErrorComponent />;
      case "type":
        return <TypeErrorComponent />;
      case "async":
        return <AsyncErrorComponent />;
      default:
        return null;
    }
  };

  return (
    <>
      {/* This will actually trigger error boundaries when errorMode is set */}
      {renderErrorComponent()}

      {/* Control panel */}
      <div className="fixed bottom-4 left-4 bg-card border border-red-500 rounded-lg p-3 shadow-lg z-50 max-w-xs">
        <div className="text-center space-y-2">
          <div className="flex items-center justify-center gap-2">
            <Server className="h-4 w-4 text-red-500" />
            <span className="text-xs font-medium">Prod Error Tests</span>
          </div>

          <div className="grid grid-cols-2 gap-1">
            <Button
              onClick={() => triggerRenderError("runtime")}
              size="sm"
              variant="destructive"
              className="h-8 text-xs"
            >
              <Bug className="h-3 w-3 mr-1" />
              Render
            </Button>

            <Button
              onClick={() => triggerRenderError("type")}
              size="sm"
              variant="outline"
              className="h-8 text-xs"
            >
              <AlertTriangle className="h-3 w-3 mr-1" />
              TypeError
            </Button>

            <Button
              onClick={() => triggerRenderError("async")}
              size="sm"
              variant="outline"
              className="h-8 text-xs"
            >
              <RefreshCw className="h-3 w-3 mr-1" />
              Async
            </Button>

            <Button
              onClick={triggerUncaughtError}
              size="sm"
              variant="outline"
              className="h-8 text-xs border-orange-500 text-orange-500"
            >
              <AlertTriangle className="h-3 w-3 mr-1" />
              Uncaught
            </Button>
          </div>

          {errorMode && (
            <div className="text-[10px] text-green-600 animate-pulse">
              Error triggered: {errorMode}
            </div>
          )}

          <p className="text-[10px] text-muted-foreground">
            Tests real error boundaries
          </p>
        </div>
      </div>
    </>
  );
}
// // components/auth/production-error-trigger.jsx

// "use client";

// import { useState } from "react";
// import { Button } from "@/components/ui/button";
// import { AlertTriangle } from "lucide-react";
// import { isDevelopment } from "@/lib/environment";

// export function ProductionErrorTrigger() {
//   // Only mount in production
//   if (isDevelopment) {
//     return null;
//   }

//   const [shouldThrow, setShouldThrow] = useState(null);

//   // Render-time throws will be caught by React/Next error boundary
//   if (shouldThrow === "runtime") {
//     throw new Error("Real production runtime error - component failure");
//   }

//   if (shouldThrow === "type") {
//     // Causes a TypeError during render
//     const nullObject = null;
//     // Accessing property during render will throw
//     // eslint-disable-next-line @typescript-eslint/no-unused-expressions
//     nullObject.someMethod();
//   }

//   return (
//     <div className="fixed bottom-4 left-4 bg-card border border-red-500 rounded-lg p-3 shadow-lg z-50">
//       <div className="text-center space-y-2">
//         <div className="flex items-center justify-center gap-1">
//           <AlertTriangle className="h-3 w-3 text-red-500" />
//           <span className="text-xs font-medium">Prod Error Test</span>
//         </div>

//         <Button
//           onClick={() => setShouldThrow("runtime")}
//           size="sm"
//           variant="destructive"
//           className="w-full h-8 text-xs"
//         >
//           Runtime Error
//         </Button>

//         <Button
//           onClick={() => setShouldThrow("type")}
//           size="sm"
//           variant="outline"
//           className="w-full h-7 text-xs"
//         >
//           TypeError
//         </Button>

//         <p className="text-[10px] text-muted-foreground">
//           Tests real error boundary
//         </p>
//       </div>
//     </div>
//   );
// }

// "use client";

// import { Button } from "@/components/ui/button";
// import { AlertTriangle } from "lucide-react";
// import { isDevelopment } from "@/lib/environment";

// export function ProductionErrorTrigger() {
//   // Don't render in development
//   if (isDevelopment) {
//     return null;
//   }

//   const triggerRealError = () => {
//     throw new Error("Real production runtime error - component failure");
//   };

//   const triggerTypeError = () => {
//     const nullObject = null;
//     nullObject.someMethod();
//   };

//   return (
//     <div className="fixed bottom-4 left-4 bg-card border border-red-500 rounded-lg p-3 shadow-lg z-50">
//       <div className="text-center space-y-2">
//         <div className="flex items-center justify-center gap-1">
//           <AlertTriangle className="h-3 w-3 text-red-500" />
//           <span className="text-xs font-medium">Prod Error Test</span>
//         </div>

//         <Button
//           onClick={triggerRealError}
//           size="sm"
//           variant="destructive"
//           className="w-full h-8 text-xs"
//         >
//           Runtime Error
//         </Button>

//         <Button
//           onClick={triggerTypeError}
//           size="sm"
//           variant="outline"
//           className="w-full h-7 text-xs"
//         >
//           TypeError
//         </Button>

//         <p className="text-[10px] text-muted-foreground">
//           Tests real error boundary
//         </p>
//       </div>
//     </div>
//   );
// }

// // components/auth/production-error-trigger.jsx
// "use client";

// import { Button } from "@/components/ui/button";
// import { AlertTriangle, Link } from "lucide-react";
// import { useSearchParams } from "next/navigation";
// import { useEffect } from "react";

// export function ProductionErrorTrigger() {
//   const searchParams = useSearchParams();

//   // Handle URL-triggered errors
//   useEffect(() => {
//     const triggerError = searchParams.get("test_error");

//     if (triggerError === "boundary") {
//       throw new Error("Production error boundary test triggered via URL");
//     }

//     if (triggerError === "runtime") {
//       throw new Error("URL-triggered runtime error in production");
//     }

//     if (triggerError === "undefined") {
//       // This will cause a runtime error
//       const obj = undefined;
//       console.log(obj.nonExistentProperty);
//     }
//   }, [searchParams]);

//   // Handle button-triggered errors
//   const triggerRealError = () => {
//     throw new Error("Real production runtime error - component failure");
//   };

//   const triggerTypeError = () => {
//     // This will cause a TypeError
//     const nullObject = null;
//     nullObject.someMethod();
//   };

//   const triggerReferenceError = () => {
//     // This will cause a ReferenceError
//     someUndefinedFunction();
//   };

//   return (
//     <div className="fixed bottom-4 left-4 bg-card border border-red-500 rounded-lg p-3 shadow-lg z-50">
//       <div className="text-center space-y-2">
//         <div className="flex items-center justify-center gap-1">
//           <AlertTriangle className="h-3 w-3 text-red-500" />
//           <span className="text-xs font-medium">Prod Error Test</span>
//         </div>

//         <Button
//           onClick={triggerRealError}
//           size="sm"
//           variant="destructive"
//           className="w-full h-8 text-xs"
//         >
//           Runtime Error
//         </Button>

//         <div className="flex gap-1">
//           <Button
//             onClick={triggerTypeError}
//             size="sm"
//             variant="outline"
//             className="flex-1 h-7 text-xs"
//           >
//             TypeError
//           </Button>
//           <Button
//             onClick={triggerReferenceError}
//             size="sm"
//             variant="outline"
//             className="flex-1 h-7 text-xs"
//           >
//             RefError
//           </Button>
//         </div>

//         <div className="flex items-center gap-1 text-[10px] text-muted-foreground">
//           <Link className="h-3 w-3" />
//           <span>URL: ?test_error=boundary</span>
//         </div>
//       </div>
//     </div>
//   );
// }

// Test Real Errors in Production:
// Via URL Parameters:
// https://yourapp.com/login?test_error=boundary
// https://yourapp.com/login?test_error=runtime
// https://yourapp.com/login?test_error=undefined
