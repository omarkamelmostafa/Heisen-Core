...
import { RefreshCw, Home, ArrowLeft, AlertCircle, Target } from "lucide-react";

// Consistent environment detection
const isDevelopment = process.env.NEXT_PUBLIC_APP_ENV === "development";

export function ErrorBoundary({ error, reset, rouletteResult }) {
  // For REAL errors (from Next.js error.js), use the actual error message
  // For simulation errors, use simulation messages
  const isSimulationError =
    error?.message?.includes("Russian Roulette") ||
    error?.title?.includes("Development Error Simulation");

  const errorMessage =
    isDevelopment && isSimulationError
      ? error?.message || "Development error occurred"
      : error?.message ||
        "Oops, something went wrong on our end. Please try again later.";

  const errorTitle =
    isDevelopment && isSimulationError
      ? error?.title || "Development Error"
      : error?.title || "Something Went Wrong";

  return (
    <div className="min-h-screen flex items-center justify-center bg-background p-6">
      <div className="max-w-md w-full text-center space-y-6">
        {/* Russian Roulette Info - Dev Only for simulation errors */}
        {isDevelopment && rouletteResult && (
        ...
        <div className="space-y-3">
          ...
          {isDevelopment && isSimulationError && (
            <p className="text-xs text-blue-600">
              🔧 This is a development simulation. In production, real errors
              would be caught here.
            </p>
          )}
          {isDevelopment && !isSimulationError && (
            <p className="text-xs text-orange-600">
              ⚠️ This is a real runtime error caught by Next.js error boundary.
            </p>
          )}
        </div>
