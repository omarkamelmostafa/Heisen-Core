...
export function useErrorSimulator() {
  ...
  // Russian Roulette only in development
  useEffect(() => {
    if (isDevelopment) {
      const runRoulette = async () => {
        ...
        if (isHit) {
          const errorData = {
            ...
            message:
              "This is a simulated error from Russian Roulette in development mode.",
          };
          setError(errorData);
        }
      };
  ...
  const triggerError = async () => {
    if (isDevelopment) {
      ...
      if (isHit) {
        const errorData = {
          title: "Development Error Simulation",
          message: "This is a manually triggered error from Russian Roulette.",
        };
        setError(errorData);
