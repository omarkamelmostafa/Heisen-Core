...
export function useErrorSimulator() {
  ...
  if (!isDevelopment) {
    return {
      ...
      triggerError: async () => ({ triggered: false }),
      clearError: () => {},
    };
  }

  // Development-only Russian Roulette
  useEffect(() => {
    const runRoulette = async () => {
      setRouletteStatus((prev) => ({ ...prev, isSpinning: true }));
      ...
      if (isHit) {
        setError({
          title: "Development Error Simulation",
          message: "Russian Roulette triggered this simulated error",
        });
      }
    };
  ...
  const triggerError = async () => {
    ...
    if (isHit) {
      const errorData = {
        title: "Development Error Simulation",
        message: "Manually triggered Russian Roulette error",
      };
      setError(errorData);
      return { triggered: true, error: errorData, chamber, trigger };
    }

    return { triggered: false, chamber, trigger };
  };
