...
│   ├── lib/
│   │   ├── config/               # api-config.js (309 LOC), auth/ (granular page content)
...
- [x] **C4**: Create a React Error Boundary component wrapping `{children}` in `layout.jsx`. Add `error.jsx` and `global-error.jsx` files in `app/` for App Router error handling.
  - *Completed On: 2026-02-20*
- [x] **C5**: Decouple services from the Redux store — remove `import { store }` from all services in `src/services/` by creating a `StoreAccessor` utility. Inject the store in `StoreProvider`.
  - *Completed On: 2026-02-20*
...
- [x] **C8**: Remove [locale] dynamic segment and implement default routing (login for unauthenticated, dashboard for authenticated)
  - *Completed On: 2026-02-20*
  - *Completed On: 2026-02-20*

Description:
The app currently uses a [locale] dynamic segment for internationalization, but i18n is not required and complicates routing and authentication flows. We need to remove the entire [locale] folder, flatten the app structure, and set up default routing:

Unauthenticated users should land on the login page (/login or /).

Authenticated users should be redirected to a new dashboard page (/dashboard).
This will simplify the codebase and align with the current auth-only state.

Location:

src/app/[locale]/ (entire directory)

src/middleware.js

Root layout (src/app/layout.jsx)

Auth components and thunks (redirect logic)

jsconfig.json (if locale aliases exist)

Impact:
Removes i18n complexity, streamlines routing, and establishes a clear authenticated/unauthenticated experience. Requires creation of a placeholder dashboard and updates to all route protections.

Subtasks:

Delete the entire src/app/[locale]/ directory (including all nested folders and files).
Move auth routes (login, signup, verify-email, forgot-password, reset-password) to a new route group src/app/(auth)/ (if not already present) and update their paths:
...
Either make the login page the index by moving (auth)/login/page.jsx to src/app/page.jsx OR keep a root page that redirects to /login if not authenticated. For simplicity, we'll make login the index (recommended).
If a landing page is desired later, we can adjust; for now, login is the entry point.
Update root layout (src/app/layout.jsx):
Remove any locale‑related props or logic.
Ensure it still wraps children with StoreProvider and any global UI.
Update middleware (src/middleware.js):
Remove locale pattern matching (e.g., /:locale/*).
Protect routes:
Public routes: /login, /signup, /verify-email, /forgot-password, /reset-password (and possibly / if login is index).
Protected routes: /dashboard/*, /app/* (if any).
...
Create src/app/dashboard/page.jsx with a simple "Dashboard" heading and a logout button (that dispatches logout).
Optionally create src/app/dashboard/layout.jsx for future dashboard layout.
Update auth thunks (auth-thunks.js):
After successful login, redirect to /dashboard (instead of any previous locale‑based path).
After logout, redirect to /login.
Fix any hardcoded links in auth components (e.g., "Go to dashboard" after email verification) to point to /dashboard.
Remove i18n‑related code:
If auth-content.js contained translations, either remove or keep only one language.
Delete any locale‑aware helpers.
Clean up jsconfig.json – remove any aliases pointing to @/locale/* or similar.
Test the flow:
Unauthenticated: visit / → see login page. All auth routes work.
Authenticated: visit / → redirect to /dashboard. Visiting /login redirects to /dashboard.
Direct access to protected routes (/dashboard) redirects to login if not authenticated.
Risk Assessment: Medium. This is a structural change that touches routing, middleware, and many pages. However, it simplifies the app and removes unnecessary complexity. Thorough testing of all auth flows is required.

### 🟡 IMPORTANT

- [x] **I1–I2**: Remove all commented-out code from `user-service.js`, `api-config.js`, `eslint.config.mjs`, `tailwind.config.js`, `store-provider.jsx`.
  - *Completed On: 2026-02-20*
- [x] **I3**: Convert `FLOW.js` to `FLOW.md` and move to a `docs/` folder (or delete if covered by this summary).
  - *Completed On: 2026-02-20*
- [x] **I4–I5**: Delete `combine.js`, `combined_data.txt`, `How-To-Use.js`, `headache.txt` from source tree. Move utility scripts to a `scripts/` folder outside `src/`.
  - *Completed On: 2026-02-20*
...
- [x] **I7**: Delete `tailwind.config.js` (Tailwind v4 uses CSS-based config in `globals.css`).
  - *Completed On: 2026-02-20*
- [x] **I8**: Clean `jsconfig.json` — remove aliases pointing to non-existent paths (`@/auth/*`, `@/dashboard/*`, `@/users/*`, `@/settings/*`, `@/forms/*`, `@/layout/*`, `@/api/*`, `@/assets/*`, `@/types/*`). Remove `../to-delete/` from `include`.
  - *Completed On: 2026-02-20*
...
- [x] **I10**: Refactor thunks to use `createAsyncThunk` with `extraReducers` (pending/fulfilled/rejected) instead of manual `dispatch(startLoading())`.
  - *Completed On: 2026-02-20*
- [x] **I11**: Update metadata in `layout.jsx` — proper title template, description, OG tags, favicon, manifest link.
  - *Completed On: 2026-02-20*
...
- [x] **I13**: Guard `NotificationService.initialize()` with `typeof window !== 'undefined'` check before accessing `localStorage` or `Notification` API.
  - *Completed On: 2026-02-20*
- [x] **I14**: Add AbortController support to thunks — pass `signal` via `thunkAPI.signal` and wire to axios requests.
  - *Completed On: 2026-02-20*
- [x] **I15**: Memoize selectors using `createSelector` from `@reduxjs/toolkit` — especially `selectAuthStatus` and compound selectors.
  - *Completed On: 2026-02-20*

### 🟢 ENHANCEMENTS

- [ ] **E1**: Set up Vitest + React Testing Library. Add unit tests for slices, services, and critical components.
- [x] **E2**: Create `createAppThunk` helper that wraps `createAsyncThunk` with standardized loading/error/success handling.
  - *Completed On: 2026-02-20*
- [x] **E3**: Use `next/dynamic` for heavy components (e.g., `AnimatedLogoLoader`, `FantasyLoader`).
  - *Completed On: 2026-02-20*
- [x] **E4**: Add `loading.jsx` files in route segments for streaming SSR.
  - *Completed On: 2026-02-20*
- [x] **E5**: Implement HttpOnly cookie flow — backend sets tokens via `Set-Cookie` header, frontend never touches raw tokens.
  - *Completed On: 2026-02-20*
- [ ] **E6**: Build `<NotificationProvider>` and `<Toast>` components that render from `ui.notifications` state.
- [x] **E7**: Remove `universal-cookie` from dependencies (only `js-cookie` is used).
  - *Completed On: 2026-02-20*
...
- [x] **E10**: Split `auth-content.js` into per-page content files or integrate a headless CMS.
  - *Completed On: 2026-02-20*
- [x] **E11**: Remove `#__next` CSS rule from `globals.css`.
  - *Completed On: 2026-02-20*
