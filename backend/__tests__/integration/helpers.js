...
export async function registerVerifyAndLogin(app, emailServiceMock, overrides = {}) {
  ...
  // Get verification token from User model
  ...
  if (loginRes.body.data?.requiresTwoFactor) {
    return {
      agent,
      loginRes,
      accessToken: undefined, // 2FA flow incomplete
      requires2FA: true,
  ...
  // Return accessToken for Bearer auth tests, user with password for verification,
  // and agent for cookie-based tests
  return {
    agent,
    loginRes,
    accessToken: loginRes.body.data.accessToken,
    requires2FA: false,
    user: { ...userData, id: loginRes.body.data.user._id || loginRes.body.data.user.id }
  };
}
